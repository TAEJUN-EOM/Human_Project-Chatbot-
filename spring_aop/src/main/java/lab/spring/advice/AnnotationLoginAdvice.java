...
@Component
@Aspect
public class AnnotationLoginAdvice {
	...
	public void beforeAdviceMethod(JoinPoint joinPoint) {
		...
		System.out.println("BeforeAdvice:" + className + "." + methodName + " 핵심 메소드 전에 공통 기능 수행...");
	...
	public void afterReturningAdviceMethod(JoinPoint joinPoint, Object retVal) {
		...
		String className = joinPoint.getTarget().getClass().getSimpleName();
		String methodName = joinPoint.getSignature().getName();
		System.out.println("AfterReturningAdvice:" + className + "." + methodName + " 핵심 메소드 전에 공통 기능 수행...");
		System.out.println("return value is [" + ((Integer)retVal)+"]");
	}//afterReturningAdviceMethod() end
	
	@AfterThrowing(pointcut = "logPointcut()", throwing = "exception")
	public void afterThrowingAdviceMethod(JoinPoint joinPoint, Exception exception) throws Exception{
		Class clazz = joinPoint.getTarget().getClass();
		//JoinPoint는 핵심모듈(Pointcut) 클래스 정보와 핵심로직 메서드 정보를 저장하고 있음
		String className = joinPoint.getTarget().getClass().getSimpleName();
		String methodName = joinPoint.getSignature().getName();
	...
	public void afterAdviceMethod(JoinPoint joinPoint) {
		...
		System.out.println("핵심 로직 메소드 정상 종료와 예외 발생 경우를 모두 처리 하는 Advice");
